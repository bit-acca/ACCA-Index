...
在讲配置环境之前不得不先讲解什么是环境，什么是操作系统，以及如何操作命令行，因此在这里作简单介绍

### 初识命令行

关于命令行，相信大部分人即使不了解也至少有点刻板印象。*茵蒂克丝觉得能来看这个教程的至少也应该自己玩过windows cmd*。用比较简单的话来说命令行是一种人与系统内核交互，让系统执行命令的工具

在图形化操作系统普及前，命令行就是用户与系统的主要接口。目前绝大多数普通电脑用户都不再需要了解命令行操作，但是对于想要精通计算魔法的人，能够使用命令行来执行系统操作是非常必要的！

### 命令行的组成

所有的命令行都是由前端的*`Terminal`*和与内核交互用的`Shell`组成的。

- Terminal全称Terminal Emulator，它提供了命令行的窗口，负责将你输入的命令传递给shell，将shell的输出显示给你。
- Shell是用户和系统内核交互的程序，负责处理接受的命令然后交给系统内核来执行

> 例如当你Windows + R 打cmd回车的时候，Windows为你用Windows Terminal(Terminal)运行了cmd.exe(Shell)

### 各系统常用的Terminal和Shell

接下来简单介绍一下主流操作系统常见的Terminal和Shell工具

> 茵蒂克丝表示如果你不理解下面这些操作系统可以只看你熟悉的<!--，或者参考[操作系统话]()-->
...
- Windows:
  - Windows Terminal
- Linux:
    对Linux而言，基本上每种图形化会带上自己的一种Terminal
  - Gnome Terminal
  - Konsole
  - iTerm2
  - ...
...
- *nix系统
  ...
  - zsh：Z Shell，功能强大、可高度自定义的 shell（个人推荐）
    - 自 macOS Catalina 开始的**默认shell**
  - fish：Friendly Interactive Shell，易用、**全平台**的 shell

### 基础Shell操作

这一章的内容主要介绍以bash为准的操作方式，推荐自己开一个bash跟着操作，如果你身边没有方便的linux设备，那么也可以用Windows的cmd代替

*鉴于我们已经知道了Shell才是命令行操作的核心，之后不会再出现类似“命令行操作”这类的词，统称为Shell命令*

#### Shell 注意要点

在介绍Shell操作前，首先要列举一些在Shell操作中需要格外注意的地方：

- `Prompt`: **命令提示符**，是一类用来**提醒你“现在可以输入内容”**的符号
- `当前路径`：也称作**工作路径**，可以理解为**Shell当前所处的位置**
  - 务必要清楚自己正在哪个路径（通俗的理解：哪个文件夹）进行操作
  - 基本所有的命令都与Shell目前所处的路径有关
- `用户`：要清楚当前在操作Shell的用户是谁
  - 这里的用户是操作系统中的概念
  - 不同的用户拥有的权限不同，会影响将要执行的操作

#### 基础命令介绍

> 茵蒂克丝：这一段可能有点抽象，希望大家去多操作多理解，不要硬看

命令分为**本体**和**参数**（改变一些执行命令的方式，在本体后打一个空格然后加上，一般来说单字母参数前加-x，多字母参数前加两杠--xxx）

如果不知道某命令如何使用，大部分命令打`<命令> -h`或者`<命令> --help`会给出这个开发者对命令的使用方式

- `pwd`： 获得当前路径
  > 路径是指类似于*D:\Files\GitHub\ACCA-Index\基础篇\第1话 编程环境与VSCode.md*的一个字符串，用于定位系统里的文件
  - *sh特有命令，cmd请使用`chdir`*
- `cd <path>`： 将Shell移动到\<path>的路径
    > \<path> 指一段路径，这种表达方式可能会在本书中多次出现
  - \<path> 可以是**绝对路径**（从盘符（对Windows是\<盘符>:\，Linux是/）开始直到你想到的地方）或者**相对路径**(对绝大多数Shell，`.`代表当前路径，`..`代表上一层路径)
- `ls`: 列出当前路径内下的目录和文件
  - `-a`: 列出所有的目录和文件，包括隐藏的文件
  - `-l`: 列出文件的详尽信息
  - *sh特有命令，cmd请使用`dir`*
- `touch <file>`: 创建一个文件，需要自己打文件后缀
- `mkdir <dir>`：创建一个目录
- `rm <file>`：删除一个文件
  - `-r`：递归删除目录
  - `-f`：强制删除
...
- `cp <源目录> <目标目录>`：复制文件或目录
  - `-r`：递归复制目录
- `mv <源目录> <目标目录>`：移动文件或目录（重命名）
  - 这个命令用于重命名只需要保持源路径和目标路径相同，只改变文件名即可。例如

    ``` shell
    mv ./Dedicatus545.txt ./Dedicatus545_parts1.txt
    # 这项操作将Dedicatus545.txt移动到同目录下的Dedicatus545_parts1.txt，等价于重命名
    ```

此处只对这些最基础命令做介绍<!--，具体请参考[命令行篇]()-->
