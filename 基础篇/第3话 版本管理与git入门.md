...
![文件的状态变化周期*（图源《Pro Git》 CC BY-NC-SA 3.0）*](./imgs/03_01_lifecycle.png)
...
虽然git没有具体规定分支的用途，但分支已经形成了固定习惯。一般来说一个项目会把分支分为`长期分支`和`主题分支`，比如主分支`master`保持基本稳定，在上面分支出`develop`分支开发，再在`develop`上分支出负责不同功能的主题分支，开发完成后逐渐向上合并。这是现代的一套典型开发流程
